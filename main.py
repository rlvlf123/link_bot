...
async def is_admin_channel(i: discord.Interaction):
    ...
    if i.channel_id != admin_ch_id:
        await i.response.send_message("❌ 이 명령어는 **관리 전용 채널**에서만 사용할 수 있습니다.", ephemeral=True)
        return False
    return True
...
async def help_command(i: discord.Interaction):
    ...
    embed.add_field(name="📜 `/내역 [별명/ID]`", value="현재 상태 및 이전 닉네임 기록 조회", inline=False)
    embed.add_field(name="📊 `/현황`", value="현재 감시 중인 전체 유저 명단 확인", inline=False)
    embed.add_field(name="❌ `/삭제 [별명/ID]`", value="감시 목록에서 유저 제거 (관리자용)", inline=False)
    embed.footer = discord.EmbedFooter(text="알림은 설정된 전용 채널로 자동 전송됩니다.")
...
async def add_user(i: discord.Interaction, steam_id: str, nickname: str = None):
    ...
    if not player: return await i.followup.send("❌ 유저 정보를 찾을 수 없습니다.")
    key = nickname or steam_id
    history = crawl_steam_history(steam_id)
    if not history or history[-1] != player['personaname']: history.append(player['person
