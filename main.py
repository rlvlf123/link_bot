...
class MyBot(commands.Bot):
    ...
    async def check_steam_nicknames(self):
        ...
        for name_key, sid, history_str, is_monitored in rows:
            ...
            # 🛠️ 버그 수정 1: XML 파싱이나 API가 None을 반환했을 때 예외 처리 추가
            if not curr_nick or str(curr_nick).strip() == "": 
                continue
            ...
            # 🛠️ 버그 수정 2: 별명(name_key)이 아닌 절대 변하지 않는 스팀 ID(sid_col) 기준으로 UPDATE 실행
            def update_db(h_str, s_id):
                conn = get_db()
                cursor = conn.cursor()
                cursor.execute(f"UPDATE users SET {hist_col} = ? WHERE {sid_col} = ?", (h_str, s_id))
                conn.commit()
                conn.close()
...
async def status_list(i: discord.Interaction):
    ...
    pages = []
    B = "```"
    current_page = f"📊 **전체 조우 현황 (🔔=감시중 / 🔇=기록만)**\n{B}text\n상태 / 별명 / 현재닉네임 / SteamID\n"
